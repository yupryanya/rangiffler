...
<div class="panel panel-info">
    <div class="panel-heading">Status Code</div>
    <div class="panel-body">
        <#if data.responseCode??>
            <div class="alert alert-${(data.responseCode?starts_with("2"))?then("success", "danger")}">
                <b>${data.responseCode}</b>
            </div>
        <#else>
            <div class="alert alert-warning">Unknown</div>
        </#if>
    </div>
</div>
...
<#if data.url??>
    <div class="panel panel-default">
        <div class="panel-heading">Request URL</div>
        <div class="panel-body"><code>${data.url}</code></div>
    </div>
</#if>
